...
function formCheck(event) {
    ...
    if (!isNumber(form.codice.value) || form.codice.value.length !== 3) {
        ...
        div.classList.remove("hidden");
        document.body.classList.add("no-scroll");
        message.textContent = "CCV non valido";
        event.preventDefault();
        return;
    }
}
...
function destroyCartCookie(){
    ...
    let exp_data="09/08/1943,19:43:00";
    exp_data=new Date(exp_data).toGMTString();
    document.cookie="cartcookie=;expires = "+exp_data;
}
...
function showEmptyCartMessage(){
    ...
    a.href="prodotti.php";
    a.addEventListener("click",destroyCartCookie);
    div.appendChild(a);
}
...
function showProducts() {
    ...
    for (product of products) {
        ...
        trashcan.src = "img/cart/delete.png";
...
fetch("fetch_productsInfo.php").then(onResponse, onError).then(onJson);
